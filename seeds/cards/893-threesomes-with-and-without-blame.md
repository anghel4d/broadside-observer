---
title: "Threesomes, With and Without Blame"
authors:
- Jeremy G. Siek
- Philip Wadler
year: 2010
venue: POPL
arxiv: null
doi: 10.1145/1706299.1706325
source: "https://doi.org/10.1145/1706299.1706325"
topics:
- gradual-typing
- casts
- type-safety
seed_rank: 893
seed_batch: to1000-db-types-2026-08-13
reviewed: 2026-08-13
pool: languages
relevance_score: 9
lineage: type-safety
cites:
  []
---

# Threesomes, With and Without Blame

## One-sentence takeaway

Space-efficient coercions (threesomes) for gradual cast calculi.

## Why it matters here

Practical cast representation needed for performant gradual runtimes.

## Key ideas

- Coercion algebra compressing cast chains.
- Blame-preserving normalization.
- Runtime efficiency for migratory typing.

## Caveats

- Part of the gradual/refinement type-safety shelf.
- Check mechanization status in follow-ons.

## Links

- DOI: [10.1145/1706299.1706325](https://doi.org/10.1145/1706299.1706325)
- URL: https://doi.org/10.1145/1706299.1706325
