---
title: Abstracting Gradual Typing
authors:
- Ronald Garcia
- Alison M. Clark
- Éric Tanter
year: 2016
venue: POPL
arxiv: null
doi: 10.1145/2837614.2837670
source: "https://doi.org/10.1145/2837614.2837670"
topics:
- gradual-typing
- abstract-interpretation
- type-safety
seed_rank: 1144
seed_batch: to1000-db-types-2026-08-13
reviewed: 2026-08-13
pool: languages
relevance_score: 9
lineage: type-safety
cites:
  []
---

# Abstracting Gradual Typing

## One-sentence takeaway

AGT derives gradual type systems from static ones via abstract interpretation.

## Why it matters here

Recipe for gradualizing ownership, effects, and refinements systematically.

## Key ideas

- Galois connections between static and gradual spaces.
- Evidence calculi from abstraction.
- Reusable methodology beyond STLC.

## Caveats

- Part of the gradual/refinement type-safety shelf.
- Check mechanization status in follow-ons.

## Links

- DOI: [10.1145/2837614.2837670](https://doi.org/10.1145/2837614.2837670)
- URL: https://doi.org/10.1145/2837614.2837670
