---
title: Shortest Connection Networks and Some Generalizations
authors:
- R. C. Prim
year: 1957
venue: Bell System Technical Journal
arxiv: null
doi: 10.1002/j.1538-7305.1957.tb01515.x
source: "https://doi.org/10.1002/j.1538-7305.1957.tb01515.x"
topics:
- mst
- prim
seed_rank: 1016
seed_batch: perfects-algorithms-2026-08-13
reviewed: 2026-08-13
pool: systems
relevance_score: 10
lineage: algorithms-and-complexity
cites:
- title: On the Shortest Spanning Subtree of a Graph and the Traveling Salesman Problem
  url: "https://doi.org/10.1090/S0002-9939-1956-0078686-7"
  year: 1956
  arxiv: null
  doi: 10.1090/S0002-9939-1956-0078686-7
  card: 1015-on-the-shortest-spanning-subtree-of-a-graph-and-the-travelin
- title: A Note on Two Problems in Connexion with Graphs
  url: "https://doi.org/10.1007/BF01386390"
  year: 1959
  arxiv: null
  doi: 10.1007/BF01386390
  card: 386-a-note-on-two-problems-in-connexion-with-graphs-dijkstra
---

# Shortest Connection Networks and Some Generalizations

## One-sentence takeaway

Prim’s MST grows a single tree by lightest outgoing edge.

## Why it matters here

Dense-graph MST workhorse; Jarník–Prim–Dijkstra family kinship.

## Key ideas

- Cut-property greedy growth.
- Heap implementations.
- Structure related to Dijkstra.
- Practical with adjacency matrices.

## Caveats

- Jarník 1930 priority historically.
- Heap choice dominates asymptotics.

## Links

- DOI: [10.1002/j.1538-7305.1957.tb01515.x](https://doi.org/10.1002/j.1538-7305.1957.tb01515.x)
- URL: https://doi.org/10.1002/j.1538-7305.1957.tb01515.x
