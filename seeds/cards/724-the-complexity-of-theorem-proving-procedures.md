---
title: The Complexity of Theorem-Proving Procedures
authors:
- Stephen A. Cook
year: 1971
venue: STOC
arxiv: null
doi: 10.1145/800157.805047
source: "https://doi.org/10.1145/800157.805047"
topics:
- complexity
- np-completeness
- sat
seed_rank: 724
seed_batch: perfects-algorithms-2026-08-13
reviewed: 2026-08-13
pool: maths-foundations
relevance_score: 10
lineage: algorithms-and-complexity
cites:
- title: Reducibility Among Combinatorial Problems
  url: "https://doi.org/10.1007/978-1-4684-2001-2_9"
  year: 1972
  arxiv: null
  doi: 10.1007/978-1-4684-2001-2_9
- title: Universal Sequential Search Problems
  url: "https://www.cs.bu.edu/fac/lnd/dvi/ussp.pdf"
  year: 1973
  arxiv: null
  doi: null
- title: "Computers and Intractability: A Guide to the Theory of NP-Completeness"
  url: "https://en.wikipedia.org/wiki/Computers_and_Intractability"
  year: 1979
  arxiv: null
  doi: null
- title: A Machine Program for Theorem-Proving
  url: "https://doi.org/10.1145/368273.368557"
  year: 1962
  arxiv: null
  doi: 10.1145/368273.368557
see:
- "725-reducibility-among-combinatorial-problems"
- "726-universal-sequential-search-problems"
- "727-computers-and-intractability-a-guide-to-the-theory-of-np-com"
- "803-a-machine-program-for-theorem-proving"
---

# The Complexity of Theorem-Proving Procedures

## One-sentence takeaway

SAT is NP-complete — the Cook–Levin theorem that founded NP-completeness.

## Why it matters here

Root hardness result behind almost every intractability argument Anoptic touches.

## Key ideas

- Polynomial-time reducibility as comparison tool.
- SAT captures nondeterministic polynomial time.
- Tableau encoding into CNF.
- Launches the NP-completeness program.

## Caveats

- Does not imply P≠NP.
- Fine structure of reductions came later.

## Links

- DOI: [10.1145/800157.805047](https://doi.org/10.1145/800157.805047)
- URL: https://doi.org/10.1145/800157.805047
