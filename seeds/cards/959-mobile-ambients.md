---
title: "Mobile Ambients"
authors:
  - "Luca Cardelli"
  - "Andrew D. Gordon"
year: 2000
venue: "Theoretical Computer Science"
arxiv: null
doi: "10.1016/S0304-3975(99)00231-5"
source: "https://doi.org/10.1016/S0304-3975(99)00231-5"
topics:
  - ambients
  - cardelli
  - gordon
  - mobility
  - concurrency
seed_rank: 959
seed_batch: "perfects-pl-2026-08-13"
reviewed: "2026-08-13"
pool: "languages"
relevance_score: 10
lineage: programming-language-foundations
cites:
  - title: "A Calculus of Mobile Processes, I"
    url: "https://doi.org/10.1016/0890-5401(92)90008-3"
    year: 1992
    arxiv: null
    doi: "10.1016/0890-5401(92)90008-3"
    card: "956-a-calculus-of-mobile-processes-i"
  - title: "The Join Calculus: A Language for Distributed Mobile Programming"
    url: "https://doi.org/10.1007/3-540-45699-6_5"
    year: 2000
    arxiv: null
    doi: "10.1007/3-540-45699-6_5"
    card: "957-the-join-calculus-a-language-for-distributed-mobile-programm"
  - title: "On Understanding Types, Data Abstraction, and Polymorphism"
    url: "https://doi.org/10.1145/6041.6042"
    year: 1985
    arxiv: null
    doi: "10.1145/6041.6042"
    card: "926-on-understanding-types-data-abstraction-and-polymorphism"
  - title: "A Calculus of Communicating Systems"
    url: "https://link.springer.com/book/10.1007/3-540-10235-3"
    year: 1980
    arxiv: null
    doi: null
    card: "044-a-calculus-of-communicating-systems"
---

# Mobile Ambients

## One-sentence takeaway

Introduces mobile ambients: nested administrative domains that move, controlling locality in mobile computation.

## Why it matters here

Ambient calculus classic — locality/mobility card beyond bare π-calculus name-passing.

## Key ideas

- Ambients as bounded places.
- In/out/open capabilities for movement and dissolution.
- Model for firewalls, agents, and administrative domains.
- Companion to π-calculus and join-calculus.

## Caveats

- Seed card from the wisdom-of-the-perfects PL haul; promote to a full `summaries/` digest before relying on fine-grained claims.
- Verify primary PDF/DOI pagination against your preferred edition before formal citation.

## Links

- DOI: [10.1016/S0304-3975(99)00231-5](https://doi.org/10.1016/S0304-3975(99)00231-5)
- URL: https://doi.org/10.1016/S0304-3975(99)00231-5
