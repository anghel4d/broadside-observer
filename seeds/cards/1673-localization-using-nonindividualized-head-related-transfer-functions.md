...
Shipping KEMAR/SADIE generic sets is a product choice with known error. Wenzel/Arruda/Kistler/Wightman quantify localization with nonindividual HRTFs vs each listener's own. Anoptic should expect cone-of-confusion errors unless you personalize (CIPIC anthropometry, ML individualization 1740).

## Key ideas

- **Own vs generic HRTF.** Controlled localization experiment, NASA/Wisconsin lineage.
- **Azimuth ok, elevation/front-back not.** Pattern that still shows up in SADIE II eval (1683).
...
1993 HRTF grids and headphone EQ are dated vs SADIE/HUTUBS. Doesn't measure near-field (1691). Does not itself ship a database (that's CIPIC/KEMAR).
