# pathfinding

**Charter.** Grid pathfinding from Dijkstra/A* through HPA*, JPS/JPS+, any-angle, navmesh/Recast, flow fields, and CPD lookup databases.

## Cards

- `196-online-graph-pruning-for-pathfinding-on-grid-maps`
- `205-a-formal-basis-for-the-heuristic-determination-of-minimum-co`
- `212-jump-point-search-pathfinding-in-4-connected-grids`
- `234-reducing-redundant-work-in-jump-point-search`
- `266-improving-jump-point-search`
- `279-understanding-goal-based-vector-field-flow-field-pathfinding`
- `281-recast-navigation-detour`
- `294-near-optimal-hierarchical-pathfinding-hpa`
- `302-simplified-3d-movement-and-pathfinding-using-navigation-mesh`
- `310-a-note-on-two-problems-in-connexion-with-graphs-dijkstra`
- `332-subgoal-graphs-for-optimal-pathfinding`
- `349-lazy-theta-any-angle-path-planning-and-path-length-analysis`
- `355-theta-any-angle-path-planning-on-grids`
- `454-jps-plus-extreme-a-star-speed-optimization-static-grids`
- `468-field-d-an-interpolation-based-path-planner`
- `476-moving-ai-lab-grid-based-path-planning-competition`
- `480-ultra-fast-optimal-pathfinding-without-runtime-search`

## Notes

- Depth 2–3 shallow pass; promote only living successors.
- Do not absorb the five deep epistemologies; `card:`-link into them when needed.

**seed_batch:** `lineage-shallow-2026-08-13` · block 454–599 (`shallow-scan`).

