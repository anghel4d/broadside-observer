...
3. Packs `seeds/canvases/*.tsx` the same way (`src/generated/canvases.json.gz.b64`, inlined; no fetch)
4. Typechecks the client
5. Emits a single-file app at `dist/index.html` (cards and canvases inlined; no extra network fetch)
...
- `query_seeds {"query":"<tokens>"}` — AND search over title, authors, topics, takeaway, lineage. Optional `id`, `topic`, `lineage`, `year_min`, `year_max`, `limit` (default 20, max 50). **0 hits** return usage + copy-pasteable calls; **1 hit** (or a unique `#123` / `107` id, or a found file stem) returns the original card Markdown verbatim; **2+ hits** return a compact list with `NEXT: call get_seed {"id":"..."}`.
- `get_seed {"id":"<card-id>"}` — original card Markdown, byte-for-byte, by file stem (no `.md`). Copy `id` from a list hit.
...
Deep links stay hash routes, e.g. `https://anghel4d.github.io/broadside-observer/#card/<file-stem>`. Cards view: `https://anghel4d.github.io/broadside-observer/?view=cards#card/<file-stem>`. Canvas view: `https://anghel4d.github.io/broadside-observer/?view=canvas#canvas/<file-stem>`.
...
- **`src/mcp/`** — stdio (and optional Streamable HTTP) tools `query_seeds` / `get_seed` over the same `Corpus × Query → [SeedCard]` morphism. Search and compact result lists use the normalized corpus. Full-card responses asynchronously read the original Markdown by its validated corpus id and return it verbatim; packaged deployments include `seeds/cards` beside the application for that purpose.
...
Deep links: `#card/<file-stem>` and `#canvas/<file-stem>`. Cards mode is `?view=cards` (omitted for List). Bibliography is always shown from `cites`. In-library chips come from `see` ∪ arxiv-join and use the same hash route.
...
- **Canvas** — third view (`?view=canvas`) for packed `seeds/canvases/*.tsx`. Same list row chrome and search/sort bar (card facets hidden). The middle pane is a Raw | Render toggle like Light/Dark; last buffer stays in `localStorage`. A canvas is shareable as `?view=canvas#canvas/<file-stem>` (hash wins over the stored buffer). Ships in the same single-file Pages bundle as List/Cards — no second host, no runtime fetch.
