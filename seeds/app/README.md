...
- **Canvas** — third view (`?view=canvas`) for packed `seeds/canvases/*.tsx`. Same list row chrome and search/sort bar (card facets hidden). The middle pane is a Raw | Render toggle like Light/Dark; last buffer stays in `localStorage`. Ships in the same single-file Pages bundle as List/Cards — no second host, no runtime fetch.
